...
| **Screening / auto-reject** | 🟢 Built | IT course, link domains, resume and phone checks in the importer; held teams are `PENDING` and never written. `event_settings.screening_enabled` still has no consumer |
...
### Screening and auto-reject — built, with two things still open

- [x] **Eligibility screening runs on every import**, in `tools/EligibilityScreening`. It adds a third outcome, `PENDING`: not imported, not rejected, and re-screened from the sheet on every run. It holds a team when no member's major contains an IT keyword, when a resume/LinkedIn/GitHub link is blank or on the wrong domain, or when a phone number is present but is not 8–15 digits. A blank resume no longer imports with a note.
- [x] **`Member N Major` is a required column** and its total absence aborts the run with exit `2`. The major is read for the check and stored nowhere.
- [x] **"Rejected" is now expressible as "held".** The design question this section used to leave open was answered by **not imported and not recorded**: a `PENDING` team writes no row and no flag, so the state cannot go stale and correcting the spreadsheet is the whole of the fix. `users` is the sign-in allowlist, so a held team cannot sign in — which is the intended effect.
- [ ] **The registrant is still never told.** There is no notification path — `notifications_log` still has no writer — so a held or rejected team learns nothing unless an organiser contacts them. The report names the team, the member and the reason in plain language precisely so that a person can do that by hand.
- [ ] **`event_settings.screening_enabled` still has no consumer.** Screening always runs; the importer does not read the column. A safety check that defaults to off is not a safety check. If that toggle is meant to gate something, decide what — do not assume it gates this.
- [ ] **The admin Participants eligibility control is still a view filter, not a policy.** `eligibilityOf(studentAddress, emailVerified)` derives `eligible` / `unverified` / `not_student` from the email domain and `users.email_verified`. It stores nothing and gates nothing, and it does not look at `resume_url`.

> **The keyword list is the part that needs a human before registration opens.** `EligibilityScreening.IT_COURSE_KEYWORDS` is 20 terms, matched as case- and punctuation-insensitive substrings, and it is printed in full in every run header. A course that is missing from it costs somebody a manual check; a term that is too broad turns the check into a rubber stamp. Review it, do not assume it.
...
6. **Review the IT keyword list before registration opens**, and decide what `event_settings.screening_enabled` is for. Screening itself is built — see [§4](#4-what-is-not-done) — and it always runs, so the toggle is still a column with nothing behind it. The open half is judgement, not code: the 20 keywords in `EligibilityScreening.IT_COURSE_KEYWORDS` decide which teams a person has to look at by hand, and there is still no way to tell a held team why they were held.
