...
### Screening and auto-reject: what exists and what does not

**There is no auto-reject filter, and `event_settings.screening_enabled` is a column with no consumer.** This comes up often enough to be worth stating plainly, because three separate things get confused for it:

1. **`TeamRow`'s validation** (in `tools/`) is the only thing that rejects a row, and its rules are **hardcoded and not configurable**. It rejects on: no team name, name over 120 chars, no member blocks, team size outside `event_settings`'s min/max, a member with no name or an over-long one, a member with no email or a malformed one, a resume/LinkedIn/GitHub value that is *present but is not a URL*, and a duplicate email inside the team. It emits a **warning, not a rejection**, for a *missing* phone, resume, LinkedIn or GitHub — see the comment on `validateUrl`, which argues the case deliberately. So "resume left blank" imports today.
2. **`AdminParticipants`' eligibility filter** (frontend) is a **view filter over already-imported rows**. `eligibilityOf(studentAddress, emailVerified)` derives `eligible` / `unverified` / `not_student` from the email domain and `users.email_verified`. It stores nothing, gates nothing, and does not look at `resume_url` at all.
3. **`event_settings.screening_enabled`** is read and written by `AdminBackendService` and surfaced in the admin Participants section — and **nothing branches on it**. The importer never reads it. `admin-participants.ts` says so in a comment.

**If an auto-reject filter is wanted, `TeamRow.validateBlock` is where it goes**, with the policy read from `event_settings` beside the size limits so it is an `UPDATE` rather than a recompile — the same pattern V6 established for `min_team_size` / `max_team_size`. Turning a warning into a rejection is a one-line change per field; the design work is deciding whether a policy flag lives per-field or as one `screening_enabled` switch, and whether a rejected team should be *not imported* or *imported and flagged*. Note that "not imported" is not currently expressible: `users` is the sign-in allowlist, so a rejected registrant simply cannot sign in, and there is no `rejected` state anywhere to report back to them.
