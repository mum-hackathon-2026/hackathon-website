...
**Coverage is not the same as evenness, and two gaps remain.** `auth.spec.ts` and `sign-in.spec.ts` cover the demo path plus one GIS test (that the client id comes from `GOOGLE_CLIENT_ID`); `signInWithGoogle` itself, the script loading and the 401/403 error branches are untested, as is the whole backend `auth/` package.
